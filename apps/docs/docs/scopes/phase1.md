...
| 7   | Optional **chat-web** — same API, browser UX        | session state, stop, visibility into payload — see repo `docs/output/chat-web.md` |
...
- **chat-cli** — minimal Node client: multi-turn chat, streaming, model name from env, dependency on Ollama only. Internal runbook: `docs/output/chat-cli.md`.
- **chat-web** — Vite + React optional extra: routes, context pipeline, Stop, Groq proxy. Internal runbook: `docs/output/chat-web.md`.
